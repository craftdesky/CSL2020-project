...
// REMOVE: const STORAGE_KEY = "airport_graph_data";

const App = () => {
  // This state will always be repopulated from sample data every load!
  const [graph, setGraph] = useState(() =>
    Graph.fromData(sampleAirports, sampleRoutes)
  );
  const [message, setMessage] = useState({ text: "", type: "" });
  const [highlightedEdges, setHighlightedEdges] = useState([]);

  // Utility to clone for "edit" and force state update
  const cloneGraph = (g) => Object.assign(Object.create(Object.getPrototypeOf(g)), g);

  // --- Graph operations (edits do NOT persist after reload) ---
